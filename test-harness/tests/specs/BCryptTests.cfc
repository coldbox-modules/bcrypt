	...
	function run( testResults, testBox ){
		...
		describe( "BCrypt Module", function(){
			beforeEach( function( currentSpec ){
				variables.bcrypt = getInstance( "bcrypt@bcrypt" );
			} );
			...
			it( "Hashes and salts a password", function(){
				...
				expect( variables.bcrypt.hashPassword( variables.passwordTestString ) ).notToBe(
					variables.bcrypt.hashPassword( variables.passwordTestString )
				);
			} );
			...
			it( "Will produce a fixed length string", function(){
				expect( len( variables.bcrypt.hashPassword( variables.passwordTestString ) ) ).toBe(
					60,
					"Database storage expects a BCrypt hash to be a 60 character string"
				);
			} );
			...
			it( "Performs at a consistent high/low speed threshold", function(){
				...
				expect( executionDuration ).toBeLT(
					750,
					"#executionDuration#ms - Password hashing must be faster than .75 seconds or it will impact the user experience on logging in too much."
				);
				expect( executionDuration ).toBeGT(
					100,
					"#executionDuration#ms -Password hashing must be slower than .1 seconds or it will be too easy to brute force in an offline attack"
				);
			} );
			...
			it( "is singleton safe and will not leak with a reasonable number of concurrent iterations", function(){
				for ( var i = 1; i <= 30; i++ ) {
					...
					getInstance( "bcrypt@bcrypt" ).hashPassword( variables.passwordTestString & i );
